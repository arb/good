...
module.exports = internals.Monitor = function (plugin, options) {
    ...
    var queMessage = function (eventType, data) {
        for (var i = 0, il = self.broadcasters.length; i < il; ++i) {
            var broadcaster = self.broadcasters[i];
            broadcaster.queue(eventType, data);
        }
    };

    var sendMessages = function (broadcasters) {
        Async.each(broadcasters, function (item, callback) {
    ...
    var logHandler = function (event) {
        ...
        sendMessages(self.instantBroadcasters);
    };
    ...
    this.broadcasters = [];
    this.delayedBroadcasters = [];
    this.instantBroadcasters = [];
    var state = {};
    ...
    for (var i = 0, il = this.settings.subscribers.length; i < il; i++) {
        ...
        if (item.remote) {
            this.delayedBroadcasters.push(broadcaster);
        }
        else {
            this.instantBroadcasters.push(broadcaster);
        }
        this.broadcasters.push(broadcaster);
    }

    Async.each(this.broadcasters, function (item, callback) {

       item.start(callback);
    }, function (error) {

        if (error) {
            throw new Error('Error starting broadcasters' + error.toString());
        }
        ...
        if (self.settings.broadcastInterval) {
            state.broadcastInterval = setInterval(function () {
                sendMessages(self.delayedBroadcasters);
